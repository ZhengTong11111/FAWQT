...
gameWindow1::gameWindow1(QWidget* parent,bool _music_on_or_off,bool _volumn_on_or_off) :
    QWidget(parent)
{
    //窗口大小
    ...
    //石头
    stonePos = QRect(120, 210 - stoneHeight, stoneWidth, stoneHeight);
    leftBarrier.push_back(QRect(stonePos.right(), stonePos.top(), 0, stoneHeight));
    floorBarrier.push_back(QRect(stonePos.left(), stonePos.top(), stoneWidth, 0));
    ...
    allRewards.push_back(reward(QRect(140, 270, reward::rewardWidth, reward::rewardHeight), RED_REWARD));
    allRewards.push_back(reward(QRect(170, 35, reward::rewardWidth, reward::rewardHeight), RED_REWARD));
    allRewards.push_back(reward(QRect(320, 75, reward::rewardWidth, reward::rewardHeight), RED_REWARD));

    //蓝奖励
    allRewards.push_back(reward(QRect(575, 550, reward::rewardWidth, reward::rewardHeight), BLUE_REWARD));
    allRewards.push_back(reward(QRect(355, 75, reward::rewardWidth, reward::rewardHeight), BLUE_REWARD));
    allRewards.push_back(reward(QRect(20, 97, reward::rewardWidth, reward::rewardHeight), BLUE_REWARD));
    allRewards.push_back(reward(QRect(500, 294, reward::rewardWidth, reward::rewardHeight), BLUE_REWARD));
    ...
    sec1=new MyTimer();
    sec1->setParent(this);
    sec1->setGeometry(410,0,25,30);
    //十位秒
    sec10=new MyTimer();
    sec10->setParent(this);
    sec10->setGeometry(385,0,25,30);
    //分钟
    minute=new MyTimer();
    minute->setParent(this);
    minute->setGeometry(355,0,25,30);
    //设置计时器
    ...
    //音乐
    gBgm_on_or_off=_music_on_or_off;
    gBgm=new GameBgm();
    gBgm->addBgmFiles(); // 添加BGM文件
    gBgm->setLooping(); // 设置循环播放模式
    if(gBgm_on_or_off)gBgm->play();

    //音效
    mySoundEffect=new MySoundEffect(this);
    if(_volumn_on_or_off==false)mySoundEffect->muteAll();
    ...
    //主要角色
    fireman = new Man(20, /*516*/516);
    icegirl = new Man(20, /*430*/430);
...
void gameWindow1::paintEvent (QPaintEvent *event) {
    ...
    QPixmap pixStone;
    pixStone.load(":/props/picture_for_begin_and_game/stone.png");
    painter.drawPixmap(stonePos, pixStone);
...
void gameWindow1::timeout() {
    ...
    if (redDoor.pos.intersects(fireman->posRect())) {
        redDoor.open = true;
        if(door1Played==0){mySoundEffect->play("door");door1Played=1;}
    } else {
        redDoor.open = false;
    }
    ...
    if (blueDoor.pos.intersects(icegirl->posRect())) {
        blueDoor.open = true;
        if(door2Played==0){mySoundEffect->play("door");door2Played=1;}
    } else {
        blueDoor.open = false;
    }
